...
@Component({
	selector: "app-view-nota-document",
	template: `
		<div class="largeVerticalDialogContainer">
			...
			<ng-container >
				<div class="pdf-container" style="width: 100%; height: calc(100% - 144px);; overflow: auto;">
					...
					<div id="notaPdfViewerCanvas" style="width: 100%;"></div>
				</div>
			</ng-container>
			...
			<div class="dialog-actions">
				<div class="infoText">
					<p class="mb-0 flexi">
						...
						Visualización de la nota aclaratoria generada.
					</p>
				</div>
...
export class ViewNotaDocumentComponent implements OnInit, OnDestroy, AfterViewInit {
	constructor(
		private snackmessage: MatSnackBar,
		private visitsService: VisitsService,
		private sanitizer: DomSanitizer,
		public dialogRef: MatDialogRef<ViewNotaDocumentComponent>,
		@Inject(MAT_DIALOG_DATA) public data: { notaId: string }
	) { }

	readonly isLoading = signal(false);
	readonly hasError = signal(false);
	readonly notaId = signal('');
	...
	async ngOnInit() {
		if (this.data?.notaId) {
			this.notaId.set(this.data.notaId);
			// Loading moved to ngAfterViewInit
		} else {
			this.showError('No se proporcionó el ID de la nota');
	...
	async loadNotaPdf() {
		...
		try {
			const response = await this.visitsService.getNotaPdf(this.notaId());
			...
			this.showError('No se pudo cargar el PDF de la nota aclaratoria');
			this.hasError.set(true);
		} finally {
	...
	private async renderPdf(buffer: ArrayBuffer): Promise<void> {
		try {
			...
			await this.waitForElement('notaPdfViewerCanvas');
			
			// Get the container div
			const containerDiv = document.getElementById('notaPdfViewerCanvas');
			if (!containerDiv) {
